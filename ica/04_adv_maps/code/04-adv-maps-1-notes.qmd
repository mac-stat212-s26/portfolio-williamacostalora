...
transform_and_plot(nc, new_crs = "EPSG:4326") #Lafayette, LA
...
ggplot() +
    ...
    geom_sf(data=mn_cities |> filter(Population>100000), mapping=aes(color=Population, size= Population))+
    ggthemes::theme_map()
...
# Fix duplicate state_name columns SAFELY (only if duplicates exist)
if (sum(names(mn_counties) == "state_name") > 1) {
  names(mn_counties)[names(mn_counties) == "state_name"] <- c("state_name1", "state_name2")
}

# Check your actual column names before proceeding
names(mn_cities)
names(mn_water)
```
...
leaflet() |>
  ...
  # Title — smaller and tucked top right
  addControl(
  ...
  # Caption — small text, bottom left, narrow width
  addControl(
